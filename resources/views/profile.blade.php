...
@section('content')
    <div class="d-flex pb-3 justify-content-between">
        ...
        <div>
            @if($profile == auth()->user()->profile)
                <a href="{{ route('user.edit') }}"><i class="icon fa-solid fa-pen-to-square fa-3x"></i></a>
            @elseif($profile->profileable->conversations->contains(function ($conversation) {return $conversation->participants->contains(auth()->user());}))
                <a class="ms-3" href="{{ route('conversation', $profile->profileable->conversations->where(function ($conversation) {return $conversation->participants->contains(auth()->user());})->first()) }}"><i class="icon fa-solid fa-comment fa-3x"></i></a>
            @endif
            @if( ($profile->profileable_type == "App\Models\Team") and ($profile->profileable->members->contains(auth()->user())))
                <a class="ms-3" href="{{ route('team', $profile->profileable) }}"><i class="icon fa-solid fa-pen-to-square fa-3x"></i></a>
            @endif
    ...
    @if($profile->photos->count())
        ...
        <div>
            <img class="w-100 image" src="data:image/jpeg;base64,{{ $photo->image }}"/>
        </div>
        @endforeach
    @else
        <div>
