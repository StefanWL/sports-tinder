...
@section('content')
    <div>
        <h1 class="heading-text fw-bold pb-3">Conversations</h1>
        @foreach($conversations as $conversation)
        <a class="row text-decoration-none conversation align-items-center py-2"  href="{{ route('conversation', $conversation) }}">
        @if($conversation->profile->photos->count())
            <div class="rounded-circle overflow-hidden ms-3 col-2" style="height:50px; width:50px; background-size: cover; background-image: url('data:image/jpeg;base64,{{ $conversation->profile->photos->first()->image }}'"></div>
        @else
        ...
        @endif
            <div class="col-9">
                <h5 class="heading-text text-decoration-none mb-0">{{ $conversation->profile->name }}</h5>
